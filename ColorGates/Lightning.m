...
//
//  Simplified a bit by Nik Aggarwal in this implementation only so be wary of that
...
-(void) draw
{
	...
	CGPoint mid = drawLightning(ccp(0,0), _strikePoint, _displacement, _minDisplacement, _seed, ccp(0, _strikePoint.x));
	
	if (_split)
		drawLightning(mid, _strikePoint2, _displacement/2, _minDisplacement, _seed, ccp(0, _strikePoint.x));
...
CGPoint drawLightning(CGPoint pt1, CGPoint pt2, int displace, int minDisplace, unsigned long randSeed, CGPoint bounds)
{	
	...
	else 
	{
		...
		mid.x += (((r % 101)/100.0)-.5)*(displace*2);
		...
		mid.y += (((r % 101)/100.0)-.5)*displace/2;
        
        
        
		
		drawLightning(pt1,mid,displace/2,minDisplace,randSeed, bounds);
		drawLightning(pt2,mid,displace/2,minDisplace,randSeed, bounds);
	}
